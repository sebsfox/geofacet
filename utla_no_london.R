...
cols <- data.frame(category = c("Lower","Same","Higher","Not compared"),
                   r = c(146, 255, 192, 166),
                   g = c(208, 192, 0, 166),
                   b = c(80, 0, 0, 166)) %>%
...
p <- ggplot(dfgraph, aes(x = Timeperiod, y = Value, group = name)) +
        ...
        theme_void() +
        labs(title = unique(dfgraph$IndicatorName))

filename = paste0(getwd(),
                  "/pngs/",
                  gsub("[^[:alnum:] ]","",Sys.time()),
                  unique(dfgraph$IndicatorName),
                  ".png")
ggsave(filename, 
       plot = p, 
       width = 33, 
